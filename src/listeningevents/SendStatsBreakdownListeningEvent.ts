...
export default class SendStatsBreakdownListeningEvent implements WebSocketEventListener {
    listeningEvent: string;

    constructor() {
        this.listeningEvent = 'sendStatsBreakdown';
    }
    ...
    callback(args: { clientId: string, eventId: string, siteCode: string, siteInfo: SiteInfo }) {
        ...
        webSocketBus.registerResultEvent(args.clientId, args.eventId, 'sendStatsBreakdown', args.siteCode, args.siteInfo);
    }
}